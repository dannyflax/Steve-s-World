...
public class JOGL2Nehe10World3D implements GLEventListener, KeyListener, MouseListener {
		...
		if(!godmode){
			...
			if(mouseHeld){
				...
				if(this.pressedKeys.contains(VK_CONTROL)){
					...
					double dis1 = Math.sqrt(Math.pow(firstClick.x + 8 - this.frame.getSize().width/2, 2) + Math.pow(firstClick.y + 30 - this.frame.getSize().height/2, 2));
					double dis2 = Math.sqrt(Math.pow((current.x - this.frame.getLocation().x) - this.frame.getSize().width/2, 2) + Math.pow((current.y - this.frame.getLocation().y) - this.frame.getSize().height/2, 2));
					...
					float x = ((current.x - this.frame.getLocation().x) - firstClick.x - 8) / 10;
					float y = ((current.y - this.frame.getLocation().y) - firstClick.y - 30) / 10;
